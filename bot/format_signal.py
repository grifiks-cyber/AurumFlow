...
def build_analysis_brief(zones: dict, bias: Optional[str]) -> Optional[str]:
    """Compact TrueTrading zones post — shows all SELL/BUY levels, no SL/TP."""
    sell_zones = zones.get("SELL", [])
    buy_zones  = zones.get("BUY", [])
    if not sell_zones and not buy_zones:
        return None

    bias_map = {"BEARISH": "🔴 BEARISH", "BULLISH": "🟢 BULLISH", "RANGE": "⚪ RANGE"}
    bias_str = bias_map.get(bias, "⚪ —") if bias else "⚪ —"

    lines = [f"📊 *XAUUSD — TrueTrading Zones* | {bias_str}\n"]

    if sell_zones:
        lines.append("💰 *Sale zones:*")
        for low, high in sell_zones[:4]:
            lines.append(f"  `{low:.0f} – {high:.0f}`")

    if buy_zones:
        lines.append("\n🔄 *Retrace zones:*")
        for low, high in buy_zones[:4]:
            lines.append(f"  `{low:.0f} – {high:.0f}`")
...
def build_analysis_brief_es(zones: dict, bias: Optional[str]) -> Optional[str]:
    """Spanish compact zones post."""
    sell_zones = zones.get("SELL", [])
    buy_zones  = zones.get("BUY", [])
    if not sell_zones and not buy_zones:
        return None

    bias_map = {"BEARISH": "🔴 BAJISTA", "BULLISH": "🟢 ALCISTA", "RANGE": "⚪ RANGO"}
    bias_str = bias_map.get(bias, "⚪ —") if bias else "⚪ —"

    lines = [f"📊 *XAUUSD — Zonas TrueTrading* | {bias_str}\n"]

    if sell_zones:
        lines.append("💰 *Precios de venta:*")
        for low, high in sell_zones[:4]:
            lines.append(f"  `{low:.0f} – {high:.0f}`")
