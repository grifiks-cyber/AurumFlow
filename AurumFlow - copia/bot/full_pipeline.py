...
from bot.format_signal import build_trade_message, build_trade_message_es
...
async def _process_analysis_content(client: TelegramClient) -> bool:
    ...
    for msg in messages:
        ...
        # Format as channel post
        post = (
            f"📊 *XAUUSD Market Analysis — Aurum Flow*\n\n"
            f"{translated}\n\n"
            f"⚡ @AurumFlowXau"
        )
...
async def _process_analysis_zones(client: TelegramClient) -> bool:
    ...
    Picks the first recommended zone that matches current bias and publishes it.
    Returns True if a zone signal was published.

    Only processes messages from TODAY to prevent old zone messages republishing
    after a DB reset. Tracks message IDs in DB (channel=ANALISIS) to never republish.
    """
    channel_id = SOURCE_CHANNELS.get("ANALISIS")
    if not channel_id or current_bias is None:
        return False
    ...
    for msg in messages:
        ...
        # ── Date filter: only process messages from today ──────────────────────
        if msg.date and msg.date.date() < today_utc:
            logger.debug(f"ANALISIS zone: skipping old msg_id={msg.id} from {msg.date.date()}")
            mark_published(msg.id, channel_id)  # mark so we stop checking it
            continue

        zones = extract_analysis_zones(msg.text)

        # Pick the side that matches bias
        if current_bias == "BEARISH":
            candidates = zones.get("SELL", [])
            direction  = "SELL"
        elif current_bias == "BULLISH":
            candidates = zones.get("BUY", [])
            direction  = "BUY"
        else:
            # RANGE — mark as seen, skip
            mark_published(msg.id, channel_id)
            continue

        if not candidates:
            # Zone message exists but no matching side — mark seen
            if zones["SELL"] or zones["BUY"]:
                mark_published(msg.id, channel_id)
            continue

        # Take the first (highest-priority) recommended zone
        zone_low, zone_high = candidates[0]
        entry = zone_low if direction == "SELL" else zone_high

        logger.info(
            f"📐 ANALISIS ZONE | {direction} zone {zone_low:.0f}/{zone_high:.0f} "
            f"| entry {entry:.0f} | BIAS={current_bias}"
        )

        # Build synthetic signal dict (same interface as SENALES signals)
        synthetic_signal = {
            "type":     direction,
            "price":    entry,
            "raw_text": msg.text,
        }
        # Pass the full zone as levels so _compute_levels() uses them
        levels = [zone_low, zone_high]

        # Entry-warning check
        entry_warning = await _check_entry_warning(synthetic_signal, levels)

        formatted    = await _format_message(synthetic_signal, levels, current_bias,
                                              entry_warning=entry_warning)
        formatted_es = build_trade_message_es(synthetic_signal, levels, current_bias,
                                               entry_warning=entry_warning)

        if not formatted:
            logger.warning("ANALISIS zone: format failed — skipping")
            mark_published(msg.id, channel_id)
            _stats["errors"] += 1
            return False

        sent = await publish_signal(formatted, message_es=formatted_es)
        mark_published(msg.id, channel_id, direction, entry)

        if sent:
            _stats["published"] += 1
            _stats["detected"]  += 1
            logger.info(f"✅ ANALISIS ZONE PUBLISHED | {direction} @ {entry:.0f}")

            # Register in result tracker
            _, _, sl_val, tp1_val, tp2_val = _compute_levels(synthetic_signal, levels)
            register_active_signal(direction, entry, sl_val, tp1_val, tp2_val)

            # Spin up TikTok video in background
            asyncio.create_task(
                _generate_tiktok(client, synthetic_signal, levels, current_bias,
                                 ai_context=formatted or "")
            )
            return True
        else:
            _stats["errors"] += 1
            logger.warning("ANALISIS zone: publish failed")
            return False
